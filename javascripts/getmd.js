...
function search(keywords){
    kw = keywords;
    var url = location.protocol + '//' + location.hostname + (isroot?'':('/'+repos))+'/search.json';
    loadXMLDoc(url);
}

function searchCallback(list){
    var txt = '<div id="back_home"><a href="/" onclick="home();return false;">'+sitetitle+'</a><span>&nbsp;›&nbsp;</span><a>搜索：'+kw+'</a></div><br>';
    var post = new Array();
    for(var i = 0; i < list.data.length; i++){
        if(decodeURIComponent(list.data[i].name).toLowerCase().indexOf(decodeURIComponent(kw).toLowerCase()) != -1){
            post.push(i);
        }
    }
    if(post.length){
        for(var j = post.length - 1; j >= 0; j--){
            txt += '<blockquote class="layui-elem-quote"><postlist><a href="'+(isroot?'':('/'+repos))+'/#!/' + encodePath(list.data[post[j]].name, true) + '">' + getPostName(list.data[post[j]].name) + '</a><div class="post_info"><span class="post_date">'+list.data[post[j]].name.split('-')[0]+'-'+list.data[post[j]].name.split('-')[1]+'-'+list.data[post[j]].name.split('-')[2]+'</span></div></postlist></blockquote>';
        }
    }
    else{
        txt += '<blockquote class="layui-elem-quote">没有找到与「'+kw+'」相关的内容。<blockquote>'
    }
    loading.style.display = 'none';
    content.innerHTML = txt;
}
...
function filterJekyllHeader(blog_text){
    var jekyllSeparator = blog_text.indexOf('---', 4);
    if(jekyllSeparator != -1){
        blog_text = blog_text.substr(jekyllSeparator+3);
    }
    return blog_text;
}

function encodePath(path, lowerCaseOnly){
